fruits = ['pomegranate', 'cherry', 'apricot', 'apple',
'lemon', 'kiwi', 'orange', 'lime', 'watermelon', 'guava',
'papaya', 'fig', 'pear', 'banana', 'tamarind', 'persimmon',
'elderberry', 'peach', 'blueberry', 'lychee', 'grape', 'date' ]

f0 = []  # empty list
for f in fruits:
    value = f.upper()
    f0.append(value)
print(f"{f0 = }\n")

#    [ value for var in iterable]
f1 = [f.upper() for f in fruits]         #   apply(func, list)
print(f"{f1 = }\n")

f2 = [f.upper() for f in fruits if f.startswith('p')]
print(f"{f2 = }\n")

f3 = [f for f in fruits if f.startswith('a')]
print(f"{f3 = }\n")

nums = [800, 80, 1000, 32, -3, 8, 18, 255, 400, 5, 5000]

n1 = [float(x) for x in nums]
print(f"{n1 = }\n")

people = [
    ('Melinda', 'Gates', 'Gates Foundation', '1964-08-15'),
    ('Steve', 'Jobs', 'Apple', '1955-02-24'),
    ('Larry', 'Wall', 'Perl', '1954-09-27'), 
    ('Paul', 'Allen', 'Microsoft', '1953-01-21'),
    ('Larry', 'Ellison', 'Oracle', '1944-08-17'),
    ('Bill', 'Gates', 'Microsoft', '1955-10-28'),
    ('Thomas', 'Kurtz', 'BASIC', '1928-02-28'),
    ('Ada', 'Lovelace', 'Analytical Engine', '1815-12-10'),
    ('Grace', 'Hopper', 'COBOL', '1906-12-09'),
    ('Mark', 'Zuckerberg', 'Facebook', '1984-05-14'),
    ('Sergey','Brin', 'Google', '1973-08-21'),
    ('Larry', 'Page', 'Google', '1973-03-26'),
    ('Linus', 'Torvalds', 'Linux', '1969-12-28'),
]

dobs = [p[-1] for p in people]
print(f"{dobs = }\n")

#  [EXPR for VAR in ITERABLE if CONDITION]

with open('../DATA/breakfast.txt') as breakfast_in:
    foods = breakfast_in.read().splitlines()

print(f"{foods = }\n")

no_spam = [f for f in foods if f != 'spam']
print(f"{no_spam = }\n")

unique_foods = set(foods)
print(f"{unique_foods = }\n")

unique_foods_sorted = sorted(unique_foods)
print(f"{unique_foods_sorted = }\n")

unique_foods_sorted = sorted(unique_foods, key=str.lower)
print(f"{unique_foods_sorted = }\n")

fruits_gen = (f.upper() for f in fruits)
print(f"{fruits_gen = }\n")

for fruit in fruits_gen:
    print(fruit)

dobs_gen = (p[3] for p in people)
print(f"{dobs_gen = }\n")

for dob in dobs_gen:
    print(dob)


